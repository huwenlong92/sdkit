...
func TestCapability(t *testing.T) {
	bus := New(nil, "sdkitgo:rt")
	capability := bus.Capability()
	if !capability.Fanout {
		t.Fatal("redis pubsub driver should support fanout")
	}
	if capability.Persistent || capability.ConsumerGrp || capability.Delay || capability.Retry {
		t.Fatalf("unexpected redis pubsub capability: %+v", capability)
	}
}

func TestNilClientErrors(t *testing.T) {
	bus := New(nil, "sdkitgo:rt")
	if err := bus.Publish(context.Background(), testEvent(t, "rt:events")); err == nil || err.Error() != "eventbus redis client is nil" {
	...
	if err == nil || err.Error() != "eventbus redis client is nil" {
