...
func NewAdapter(cfg Config) (*Adapter, error) {
	...
	case ClientModeDynamic:
		if cfg.ClientLoader == nil {
			return nil, fmt.Errorf("%w: alipay client loader is required", payment.ErrAdapterNotFound)
		}
	case ClientModeStatic:
		if cfg.Client == nil {
			return nil, fmt.Errorf("%w: alipay client is required", payment.ErrAdapterNotFound)
		}
	default:
		return nil, fmt.Errorf("%w: alipay client mode %s", payment.ErrInvalidRequest, mode)
	}
	return &Adapter{
		client:       cfg.Client,
...
func (a *Adapter) CreatePayment(ctx context.Context, req payment.CreatePaymentRequest) (*payment.CreatePaymentResponse, error) {
	...
	if resp == nil {
		return nil, fmt.Errorf("%w: nil alipay create payment response", payment.ErrPaymentActionRequired)
	}
...
func (a *Adapter) QueryPayment(ctx context.Context, req payment.QueryPaymentRequest) (*payment.QueryPaymentResponse, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w: alipay query payment", payment.ErrUnsupportedCapability)
	}
...
func (a *Adapter) ClosePayment(ctx context.Context, req payment.ClosePaymentRequest) error {
	...
	if !ok {
		return fmt.Errorf("%w: alipay close payment", payment.ErrUnsupportedCapability)
	}
...
func (a *Adapter) Refund(ctx context.Context, req payment.RefundRequest) (*payment.RefundResponse, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w: alipay refund", payment.ErrUnsupportedCapability)
	}
...
func (a *Adapter) QueryRefund(ctx context.Context, req payment.QueryRefundRequest) (*payment.QueryRefundResponse, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w: alipay query refund", payment.ErrUnsupportedCapability)
	}
...
func (a *Adapter) ParseNotify(ctx context.Context, req payment.NotifyRequest) (*payment.NotifyResult, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w: alipay notify", payment.ErrUnsupportedCapability)
	}
...
func (a *Adapter) clientFor(ctx context.Context, merchantKey string) (Client, ClientCleanup, error) {
	if a.clientLoader != nil {
		...
		if client == nil {
			cleanupClient(cleanup)
			return nil, cleanup, fmt.Errorf("%w: alipay client loader returned nil client", payment.ErrAdapterNotFound)
		}
		return client, cleanup, nil
	}
	if a.client == nil {
		return nil, nil, fmt.Errorf("%w: alipay client is required", payment.ErrAdapterNotFound)
	}
...
func capabilities(cfg Config) payment.Capabilities {
	...
	return payment.Capabilities{
		...
		SupportedActions:      []payment.ActionType{payment.ActionSDKParams, payment.ActionHTMLForm, payment.ActionRedirectURL},
...
func validateAction(channel payment.Channel, action payment.ActionType) error {
	...
	case payment.ChannelAlipayWap, payment.ChannelAlipayPage:
		if action == payment.ActionHTMLForm || action == payment.ActionRedirectURL {
			return nil
		}
	default:
		return fmt.Errorf("%w: alipay channel %s", payment.ErrUnsupportedChannel, channel)
	}
	return fmt.Errorf("%w: alipay channel %s action %s", payment.ErrInvalidAction, channel, action)
}
