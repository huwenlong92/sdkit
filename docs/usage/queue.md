...
`core/queue` 只提供 `Outbox` 接口、`OutboxTask` 和 `OutboxPoller`，不提供固定 DB 表模型或 Gorm store。业务侧需要在应用仓库实现 `queue.Outbox`，并用自己的模型迁移管理 outbox 表。
...
- `Save` / `SaveBatch` 会在业务事务内写业务侧 outbox 表，会增加事务耗时和数据库写入压力。
...
- `pending` / `failed` 记录有排障和补偿价值；`sent` 记录只是投递一致性缓冲，通常只保留短期审计窗口。
...
core 当前提供：`Outbox` 接口、`OutboxTask`、常驻 poller 和重复任务语义约定。`Save`、`SaveBatch`、`Flush`、DB 迁移、清理归档和测试由业务侧 outbox store 实现。
