...
```go
auth.Default = auth.NewWithGuard(
    auth.NewSessionGuard(session.GetStore(), session.SessionTTL),
    adminauth.NewHooks(database.DB),
)
```

Session 结构和存储统一通过 `core/session` 使用。

登录成功写入 `sid` cookie，后续请求携带 cookie。
