...
`core/ratelimit` 用于 HTTP 接口限流，当前主要通过 Gin 中间件使用。常用维度包括 IP、用户、用户 + 路由；常用策略包括令牌桶、滑动窗口、固定窗口、漏桶和 BBR 自适应限流。
...
authorized.Use(authgin.AuthMiddleware(auth.Default))
...
// === Per-User（须在 authgin.AuthMiddleware(auth.Default) 之后注册） ===
...
`core/ratelimit/config.go` 提供基础配置结构，可用于业务配置映射：
...
- 用户维度限流必须注册在鉴权中间件之后。
- 用户维度限流通过 `authgin.GetIdentity(c)` 读取 `SubjectType` 和 `SubjectID`。
