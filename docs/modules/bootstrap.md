...
Filesystem 这类多服务通用 framework runtime 由 `infra/capabilities/*` 持有生命周期和默认实现；EventBus、Realtime 和 Queue producer 使用对应 `core/*/facade`。服务侧只保留业务 adapter，例如 `infra/notify`、`infra/storage`、`infra/realtime`。
