...
当前 driver 是 Asynq + Redis。运行时 driver 位于 `pkg/queue/asynq`，业务代码和 `core/queue` 公共接口禁止暴露 `asynq.Client`、`asynq.Task`、`asynq.Option`、`asynq.TaskInfo` 等类型。
...
| `unique` | yes |
| `priority` | no |
| `rate_limit` | queue control |
| `inspector` | yes |
| `pause_resume` | yes |
| `schedule` | no，使用 `crontab` 触发后投递队列 |
| `batch` | yes |
| `lock` | Redis/Memory |
...
- `EventPublisher`：用于把 runtime event 推给 dashboard、SSE、WebSocket、audit stream 等外部观测系统。publisher 必须轻量，不应阻塞 worker handler；耗时逻辑应自行异步化。
- `Observer`：用于聚合自定义 metrics、audit、runtime health 或内存态统计。observer 只观察执行结果，不修改重试、deadletter 或业务结果。
