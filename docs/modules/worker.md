...
- `runtime.go`：调用 `core/queue.RuntimeKernel`，注入 Redis、DB、Gorm Outbox、失败日志 writer 和重试间隔。
- `failure_writer.go`：把 `queue.Failure` 写入 `models.SystemQueueFailureLog`。
- `retry.go`：业务重试策略；通用 deadline/rate limit 退避回落到 `queue.DefaultRetryDelay`。

## 失败日志模型

`system_queue_failure_log` 是 PostgreSQL 按 `created_at` 月分区的失败日志表。字段包括：

- `task_id`、`queue`、`type`
- `payload`、`error`
- `retry_count`、`max_retry`、`rate_limited`
- `track_id`、`request_id`、`trace_id`、`span_id`
- `created_at`、`updated_at`

写入链路：

1. Asynq 任务执行失败。
2. `core/queue` 转换为 provider-agnostic `queue.Failure`。
3. `queue.LogFailureHandler` 输出标准日志，不打印 payload。
4. `queue.RuntimeKernel` 持有的 `queue.FailureLogger` 异步批量写入数据库。

`FailureLogger` 在 worker shutdown 时由 `RuntimeKernel.Close()` cancel 并 flush 剩余日志。
...
Asynq task headers 负责透传 `traceparent`、`tracestate`、`baggage`、`X-Track-ID`、`X-Request-ID`。worker 失败日志入库时会从 headers 提取 `track_id/request_id/trace_id/span_id`。
...
| 场景 | 验证点 |
|------|--------|
| 普通失败重试 | 多次失败会按 retry_count 写入 DB |
| 快速重试 | `source=retry_fast` 使用 200ms 间隔，便于测试 |
| 限流错误 | `queue.RateLimited` 标记 `rate_limited=true` |
...
| tracing | enqueue/worker span 自动生成，失败日志记录 trace 字段 |

## 复盘记录

已实现：

- worker 真实 DB/Redis demo 覆盖普通重试、快速重试、显式 `queue.RateLimited`、Redis `RateLimiter`、唯一任务、固定 TaskID 删除后重投、Outbox flush、失败日志入库和 tracing。
- 失败日志入库记录 `track_id/request_id/trace_id/span_id`。
...
- Queue Runtime Kernel ownership 已收敛到 `core/queue.RuntimeKernel`，worker bootstrap 不再持有 failure logger goroutine、outbox poller cancel 和 registry middleware pipeline。
...
- 2026-05-15：Queue Runtime Kernel ownership 收敛，worker bootstrap 仅注入宿主依赖，failure logger、outbox poller、dispatcher middleware pipeline 由 `core/queue` 持有。
...
- 2026-05-12：新增 worker 真实集成 demo，覆盖重试、限流、唯一任务、固定 TaskID 删除后重投、失败日志入库和 tracing。
- 2026-05-12：失败日志表补充 `track_id/request_id/trace_id/span_id`，便于从数据库反查链路。
