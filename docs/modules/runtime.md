...
- `ServiceRegistry[T]`：按 service type 注册工厂、group、dependencies 并构建服务
- `ServiceBootstrap[T]`：组合 `ServiceRegistry[T]`、`services` 配置加载和单服务配置解析，给业务 bootstrap 提供薄封装
- `ServiceRunner[T]`：按一次进程启动计划选择服务、注册进程级 capability、收集 service-local capability，并把服务定义上的 group/dependencies 带入 runtime provider
...
- 通过 `ServiceKind(serviceType)`、`ServiceGroup(serviceType)`、`ServiceDependencies(serviceType)` 暴露已注册服务类型的元信息，供上层按 selection 装配公共 capability，避免按服务名称硬编码。
...
`ServiceBootstrap[T]` 用于把这些重复装配规则收敛到 core runtime，但不接管业务配置结构。业务 bootstrap 只需要传入两个函数：

- `ServiceSpecLoader`：读取完整 `services` map，用于构建全部服务。
- `ServiceSpecResolver`：按服务名读取单个 `ServiceSpec`，用于只启动单服务时避免加载无关配置；没有这个需求时可用 `NewServiceBootstrap`。

业务层仍保留自己的 `Config`、配置文件路径和对外 facade。推荐业务 bootstrap 只做类型别名、配置读取函数和委托：

```go
var services = runtime.NewServiceBootstrapWithResolver[*Config](loadServiceSpecs, loadServiceSpec)

func RegisterProvider(provider Provider) error {
	return services.RegisterProvider(provider)
}

func BuildService(configFile string, name string, serviceType string, base *Config) (Service, error) {
	return services.BuildService(configFile, name, serviceType, base)
}
```

这样 core 统一维护 service 注册、构建、runtime capability scope 和 service kind 规则；业务 bootstrap 不再直接持有 `ServiceRegistry`、`ServiceApp` 和配置解析细节。

`ServiceRunner[T]` 用于彻底去掉业务 bootstrap 的全局 service registry。业务项目可以把 provider 和全局能力作为启动参数传入：
...
runner := runtime.NewServiceRunner(runtime.ServiceRunnerOptions[*Config]{
	...
	BaseConfig: configCapability.Config,
})
```

一个 `ServiceRunner` 实例表示一次进程启动计划：`Providers` 决定当前二进制支持哪些服务，`ServiceRunOptions.Services` 决定本次实际启动哪些服务，`Capabilities` 决定当前进程的全局能力。服务自己的 `RuntimeCapabilities` 仍由 provider 声明，只在对应服务被选择时注册。
...
如果使用 `ServiceBuilder[T]`，同样把服务元信息写在 provider 里：

```go
app.Service("worker").
	Kind(runtime.ServiceKindQueue).
	Group(runtime.GroupWorker).
	RequireCapabilities("eventbus", "redis").
	FactoryContext(factory)
```

`ServiceDefinition.Dependencies` / `ServiceBuilder.RequireCapabilities` 表示该服务启动前必须存在的公共或服务本地 capability。可选依赖使用 `OptionalCapabilities(...)` 或显式传入 `runtime.Optional(name)`。
...
bootstrap 的 `ServiceBuilder.RuntimeCapabilities(...)` 是项目服务接入这套机制的默认入口。它会把服务本地能力标记为 `ScopeServiceLocal`，并要求能力名使用服务命名空间，例如 `api.openai`、`admin.payment`、`worker.oss`。`ServiceRunner` 会自动把这些服务本地能力追加到对应 provider dependency，并在服务构建前把已注册实例写入 `ServiceContext.Capabilities`。
...
Runtime 提供两个默认 command：

- `runtime.NewServeCommand()`：执行全部 Provider。
- `runtime.NewRunCommand()`：执行指定 Provider。

默认 command 只表达 runtime 生命周期语义，不绑定 Cobra，也不读取业务配置。对于 runtime-managed provider，默认 command 会等待 runtime context 结束，避免长期服务启动后立即返回。
...
Command 不直接关闭 server、queue、cron 或底层资源。默认 `run` / `serve` command 只调用 runtime provider 生命周期；普通 provider 走 `runtime.RunProvider` / `runtime.RunAllProviders`，runtime-managed provider 走 `app.RunProvider` / `app.RunAllProviders` 后等待 runtime context 结束。
...
- Phase 4.1：新增 `runtime.NewRunCommand` / `runtime.NewServeCommand` 默认 command，业务仓库不再需要自定义 runtimecmd 包。
...
- Phase Runtime-Capability-2：bootstrap `ServiceBuilder.RuntimeCapabilities` 接入 provider-local capability，`ServiceRunner` 自动合并服务本地 capability dependency 并注入 `ServiceContext.Capabilities`。
- Phase Service-Definition-1：`ServiceDefinition` / `ServiceBuilder` 增加 service group 和 dependencies 元信息，`ServiceRunner` 选择服务时直接携带这些信息，业务启动入口不再维护服务名称白名单或依赖 switch。
