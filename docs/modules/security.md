...
- `ClientSliderProvider` / `ClientClickProvider`：临时一次性客户端通过型 Provider，分别使用 `client-slider` / `client-click` kind，仅用于过渡。
...
- 2026-05-29：临时客户端通过型 Provider 改为独立 `client-slider` / `client-click` kind，避免与正式 `slider` / `click` Provider 冲突。
